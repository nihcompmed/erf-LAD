...
if s == 0:
    cells = np.array(range(5470))[xyz[cells_tv,0] <= boundary[s]]

if s != 0:
    cells = np.array(range(5470))[(xyz[cells_tv,0] > boundary[s-1]) & (xyz[cells_tv,0] <= boundary[s])]
...
# modify PATH accordingly
with open('./pickles/patch/patch_%s_%s_%s_%s.pkl' % (str(t),str(s),str(g),str(i)), 'wb') as f:
    pickle.dump(res, f)