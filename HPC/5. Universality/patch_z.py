...
for sec in range(s):
    ...
    for g in range(10):
        ...
        for i in range(99):
            # modify PATH accordingly
            with open('./pickles/patch_z/patch_%s_%s_%s_%s.pkl' % (str(t), str(sec), str(g), str(i)), 'rb') as f:
                res = pickle.load(f)
            w[:,i:(i+1)] += res[0]
            bias[i] += res[1]
        w_list.append(w)
    ...
    if sec == 0:
        cells = np.array(range(608))[xyz[cells_te,2] <= boundary[sec]]
    if sec != 0:
        cells = np.array(range(608))[(xyz[cells_te,2] > boundary[sec-1]) & (xyz[cells_te,2] <= boundary[sec])]
    ...
    # modify PATH accordingly
    with open('./pickles/LAD/LAD_patch_z_%s_%s.pkl' % (str(t), str(sec)), 'wb') as f:
        pickle.dump(dic, f)
